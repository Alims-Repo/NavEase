...
//        generated = true
    ...
    private fun generateAutoRegisterExtension(
        ...
        val content = buildString {
            ...
            appendLine(" *")
            appendLine(" * - **JVM/Android/Desktop**: initialised automatically via `Class.forName` triggered")
            appendLine(" *   inside the runtime when [NavEaseHost] first composes.")
            appendLine(" * - **iOS/Native**: call [navEaseBootstrap] once from your platform entry point")
            appendLine(" *   (e.g. inside `ComposeUIViewController { navEaseBootstrap(); App() }`).")
            appendLine(" * - **JS/WasmJS**: initialised when the module is first loaded by the runtime.")
            appendLine(" */")
            appendLine("private object NavEaseAutoInit {")
            appendLine("    init {")
            appendLine(addEntryCalls)
            appendLine("        // Register a re-trigger hook so ensureInitialized() can call back")
            appendLine("        // into this module without a direct import (used on JVM fast-path).")
            ...
            appendLine()
            appendLine("/**")
            appendLine(" * Top-level holder for [NavEaseAutoInit].")
            appendLine(" *")
            appendLine(" * - On **JVM/Android/Desktop**: [NavEaseAutoRegistry.ensureInitialized] loads this")
            appendLine(" *   file's class via `Class.forName`, which triggers the JVM static initialiser")
            appendLine(" *   and runs `NavEaseAutoInit.init {}` — registering all screens automatically.")
            appendLine(" * - On **JS/WasmJS**: initialized at module load time — no manual call required.")
            appendLine(" * - On **iOS/Native**: accessing this property via [navEaseBootstrap] triggers")
            appendLine(" *   `NavEaseAutoInit.init {}`. Call [navEaseBootstrap] once from your platform")
            appendLine(" *   entry point before the first composition.")
            appendLine(" */")
            ...
            appendLine(" *")
            appendLine(" * - **Android / JVM / Desktop**: not required — NavEase bootstraps automatically.")
            appendLine(" * - **iOS / Native**: call this once from your platform entry point:")
            appendLine(" *   ```kotlin")
            ...
            appendLine(" *   }")
            appendLine(" *   ```")
            appendLine(" * - **JS / WasmJS**: not required — module-level init handles it automatically.")
            appendLine(" *")
            appendLine(" * Calling this function is idempotent — safe to call multiple times.")
            if (startEntry != null) {
                appendLine(" *")
            ...
            appendLine("    @Suppress(\"UNUSED_EXPRESSION\")")
            appendLine("    _navEaseAutoInit   // idempotent — NavEaseAutoInit.init{} only runs once")
            append("}")
        }
