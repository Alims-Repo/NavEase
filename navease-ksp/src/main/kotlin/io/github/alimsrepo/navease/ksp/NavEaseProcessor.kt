...
//        generated = true
    ...
    private fun generateAutoRegisterExtension(
        ...
        val content = buildString {
            ...
            appendLine(" * - **iOS/Native**: initialised eagerly via `@EagerInitialization` when the")
            appendLine(" *   Kotlin framework is loaded — no manual call required.")
            ...
            appendLine("}")
            appendLine()
            appendLine("// @EagerInitialization ensures this runs at framework-load time on")
            appendLine("// Kotlin/Native (iOS), eliminating the need for a manual navEaseBootstrap()")
            appendLine("// call from platform entry points.  On JVM/Android this is a no-op —")
            ...
            appendLine(" * Auto-generated by NavEase KSP — do not edit.")
            appendLine(" *")
            appendLine(" * **No longer required** — NavEase now bootstraps itself automatically on")
            appendLine(" * all platforms:")
            appendLine(" *")
            appendLine(" * - **Android / JVM**: class-loading via `Class.forName` inside [NavEaseHost].")
            appendLine(" * - **iOS / Native**: `@EagerInitialization` triggers at framework-load time.")
            appendLine(" * - **JS / WasmJS**: module-level initialisation at module load time.")
            appendLine(" *")
            appendLine(" * This function is kept for backward compatibility only. Calling it is safe")
            appendLine(" * (idempotent) but unnecessary — you can safely remove any existing calls.")
            if (startEntry != null) {
                appendLine(" *")
