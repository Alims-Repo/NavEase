 ...
 *
 * - **Android / JVM / Desktop**: [NavEaseHost] loads the generated class via `Class.forName`
 *   on first composition, which triggers `NavEaseAutoInit.init {}`.
 * - **iOS / Native**: the generated `_navEaseAutoInit` property is annotated with
 *   `@EagerInitialization`, so it runs when the Kotlin framework is loaded — before
 *   any composition starts.
